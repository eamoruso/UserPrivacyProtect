...
#include "pch.h"
#include <iostream>
#include <ctime>        // Include this for timing operations
#include <fstream>      // Include this to handle file operations
#include <stdio.h>
#include <windows.h>
#include <detours.h>

#define ENABLE_TIMER 1  // Used in main for tracking time taken
...
{
    std::cout << "Starting DLL injection process..." << std::endl;

#if ENABLE_TIMER // Timer Stuff
    // Get the current time before executing the external program
    ...
    std::cout << "\nTotal time taken: " << elapsedTime << " milliseconds" << std::endl;
    std::cout << "Ending Inject process..." << std::endl;

    // Save the timing results to a text file
    std::ofstream outfile("RRP_Timings.txt", std::ios_base::app);
    outfile << "Total time taken: " << elapsedTime << " milliseconds" << std::endl;
    outfile.close();
#endif 
